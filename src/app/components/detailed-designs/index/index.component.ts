...
export class IndexComponent {
  ...
  index(){
    ...
    this.mDetailedDesignService.allItems().subscribe({
      next: (response) => {
        if(response){
  ...
  onChangePage(item:any){
    ...
    this.mProgress.set(true);
    this.mDetailedDesignService.paginateItems(item).subscribe({
      next: (response) => {
        if(response){
    ...
    swalWithBootstrapButtons.fire({
      ...
      // cancelButtonColor: '#3490dc'
          ...
          if (result.isConfirmed) {
            this.mProgress.set(true);
            this.mDetailedDesignService.deleteItem(item).subscribe({
              next: (response) => {
                if(response){
                  // console.log(response)
  ...
  onSubmit(formValues: any){
    this.mProgress.set(true);
    this.mDetailedDesignService.searchItems(formValues).subscribe({
      next: (response) => {
        if(response){
